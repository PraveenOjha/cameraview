 ...
 *  * Last modified 20/09/25, 2:26 am
...
public abstract class CameraBaseEngine extends CameraEngine {
    ...
    private com.otaliastudios.cameraview.CameraView.AudioInput mAudioInput
            = com.otaliastudios.cameraview.CameraView.AudioInput.DEFAULT;
    ...
    public final void takeVideo(final @NonNull VideoResult.Stub stub,
        ...
        getOrchestrator().scheduleStateful("take video", CameraState.BIND, new Runnable() {
            ...
            public void run() {
                ...
                stub.facing = mFacing;
                stub.audio = mAudio;
                stub.setAudioInput(mAudioInput);
                stub.setAudioVolume(mAudioVolume);
    ...
    // Added: FD-based snapshot recording
    @Override
    public final void takeVideoSnapshot(@NonNull final VideoResult.Stub stub,
                                        @NonNull final FileDescriptor fileDescriptor) {
        getOrchestrator().scheduleStateful("take video snapshot fd", CameraState.BIND,
                new Runnable() {
            @Override
            public void run() {
                LOG.i("takeVideoSnapshotFD:", "running. isTakingVideo:", isTakingVideo());
                stub.fileDescriptor = fileDescriptor;
                stub.isSnapshot = true;
                stub.videoCodec = mVideoCodec;
                stub.audioCodec = mAudioCodec;
    ...
    //region Audio input routing implementation
    @Override
    public void setAudioInput(@NonNull com.otaliastudios.cameraview.CameraView.AudioInput input) {
        mAudioInput = input;
    }

    @NonNull
    @Override
    public com.otaliastudios.cameraview.CameraView.AudioInput getAudioInput() {
        return mAudioInput;
    }
    //endregion
}
