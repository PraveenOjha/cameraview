 ...
 *  * Last modified 19/09/25, 9:37 pm
...
public class CameraUtils {
    ...
    /**
     * Writes the given data to the given content Uri using the ContentResolver.
     * Runs on background thread and posts result on UI thread.
     */
    public static void writeToUri(@NonNull final Context context,
        ...
        WorkerHandler.execute(new Runnable() {
            @Override
            public void run() {
                boolean ok = writeToUriSync(context.getContentResolver(), data, uri);
                ui.post(new Runnable() {
                    @Override
                    public void run() {
                        callback.onUriResult(uri, ok);
                    }
                });
            }
        });
    ...
    /** Synchronous write to Uri. */
    @WorkerThread
    public static boolean writeToUriSync(@NonNull ContentResolver resolver,
                                         @NonNull byte[] data,
                                         @NonNull Uri uri) {
        try (OutputStream os = resolver.openOutputStream(uri, "w");
             BufferedOutputStream bos = os != null ? new BufferedOutputStream(os) : null) {
            if (bos == null) return false;
            bos.write(data);
            bos.flush();
            return true;
        } catch (Exception e) {
            LOG.e("writeToUri:", "could not write uri.", e);
            return false;
        }
    }

    /**
     * Copies a file into the given content Uri using the ContentResolver.
     * Runs on background thread and posts result on UI thread.
     */
    public static void copyFileToUri(@NonNull final Context context,
                                     @NonNull final File src,
                                     @NonNull final Uri dest,
                                     @NonNull final UriCallback callback) {
        final Handler ui = new Handler();
        WorkerHandler.execute(new Runnable() {
            @Override
            public void run() {
                boolean ok = copyFileToUriSync(context.getContentResolver(), src, dest);
                ui.post(new Runnable() {
                    @Override
                    public void run() {
                        callback.onUriResult(dest, ok);
                    }
                });
            }
        });
    ...
    /** Synchronous copy file to Uri. */
    @WorkerThread
    public static boolean copyFileToUriSync(@NonNull ContentResolver resolver,
                                            @NonNull File src,
                                            @NonNull Uri dest) {
        try (InputStream is = new java.io.FileInputStream(src);
             OutputStream os = resolver.openOutputStream(dest, "w")) {
            ...
            LOG.e("copyFileToUri:", "could not copy to uri.", e);
            return false;
        }
    }
