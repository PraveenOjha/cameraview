...
public class CameraView extends FrameLayout implements LifecycleObserver {
    ...
    private AudioInput mAudioInput = AudioInput.DEFAULT;
    ...
    private void initialize(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        // Provide a global app context for lower-level components (e.g., encoders)
        GlobalContextProvider.init(context.getApplicationContext());
    ...
    private void doInstantiateEngine() {
        ...
        // ensure engine knows current audio input preference
        mCameraEngine.setAudioInput(mAudioInput);
        // and audio volume
        mCameraEngine.setAudioVolume(mAudioVolume);
    }
    ...
    public void setEngine(@NonNull Engine engine) {
        ...
        // also re-apply audio input selection
        mCameraEngine.setAudioInput(mAudioInput);
        // and audio volume selection
        mCameraEngine.setAudioVolume(mAudioVolume);
    ...
    /**
     * Sets desired audio input routing for upcoming recordings.
     * @param input the desired audio input (DEFAULT, BLUETOOTH, USB, NONE)
     */
    public void setAudioInput(@NonNull AudioInput input) {
        mAudioInput = input;
        if (mCameraEngine != null) mCameraEngine.setAudioInput(input);
    }

    /**
     * Returns the desired audio input routing.
     */
    @NonNull
    ...
    /**
     * Sets desired audio input volume (0..100) for upcoming recordings.
     */
    public void setAudioVolume(int percent) {
        if (percent < 0) percent = 0;
        if (percent > 100) percent = 100;
        mAudioVolume = percent;
        if (mCameraEngine != null) mCameraEngine.setAudioVolume(percent);
    }

    /**
     * Returns desired audio input volume (0..100).
     */
    ...
    /**
     * Starts recording a fast, low quality video snapshot. Video will be written to the given file descriptor,
     * so callers should ensure they have appropriate permissions to write to the file.
     *
     * Throws an exception if API level is below 18, or if the preview being used is not
     * {@link Preview#GL_SURFACE}.
    ...
    public void takeVideoSnapshot(@NonNull FileDescriptor fileDescriptor) {
        VideoResult.Stub stub = new VideoResult.Stub();
        mCameraEngine.takeVideoSnapshot(stub, fileDescriptor);
        mUiHandler.post(new Runnable() {
            @Override
            public void run() {
                mKeepScreenOn = getKeepScreenOn();
