 ...
 *  * Last modified 20/09/25, 3:26 am
...
import com.otaliastudios.cameraview.CameraLogger; // added
import com.otaliastudios.cameraview.VideoResult;
import com.otaliastudios.cameraview.controls.Audio; // added
import com.otaliastudios.cameraview.controls.AudioCodec; // added
import com.otaliastudios.cameraview.controls.VideoCodec; // added
import com.otaliastudios.cameraview.internal.DeviceEncoders; // added
import com.otaliastudios.cameraview.internal.CamcorderProfiles;
import com.otaliastudios.cameraview.size.Size;
import com.otaliastudios.cameraview.CameraView; // added
...
public abstract class FullVideoRecorder extends VideoRecorder {
    ...
    private AudioManager mAudioManager; // added
    private boolean mBluetoothScoStarted = false; // added
    ...
    private boolean prepareMediaRecorder(@NonNull VideoResult.Stub stub,
        ...
        boolean hasAudio = audioChannels > 0
                && stub.audioInput != CameraView.AudioInput.NONE
                && stub.getAudioVolume() > 0;
        if (hasAudio) {
            int audioSource = MediaRecorder.AudioSource.CAMCORDER;
            if (stub.audioInput == CameraView.AudioInput.BLUETOOTH) {
                audioSource = MediaRecorder.AudioSource.VOICE_COMMUNICATION;
                // Start Bluetooth SCO to route mic
                Context ctx = com.otaliastudios.cameraview.GlobalContextProvider.getContext();
                if (ctx != null) {
            ...
            } else if (stub.audioInput == CameraView.AudioInput.USB) {
                if (Build.VERSION.SDK_INT >= 24) {
                    audioSource = MediaRecorder.AudioSource.UNPROCESSED;
                } else {
                    audioSource = MediaRecorder.AudioSource.MIC;
                }
                // NOTE: exact USB device selection is device-specific and not handled here.
            }
    ...
    protected void onStop(boolean isCameraShutdown) {
        ...
        if (mBluetoothScoStarted && mAudioManager != null) {
            try {
