...
public class SnapshotGlPictureRecorder extends SnapshotPictureRecorder {
    ...
    static public Filter customFilter;
    static public Boolean custom=false;

    // Added: controlled API for override filter used only for picture snapshots
    public static void setOverrideFilter(@Nullable Filter filter) {
        custom = filter != null;
        customFilter = filter;
    }

    @Nullable
    public static Filter getOverrideFilter() {
        return custom ? customFilter : null;
    }
    ...
    protected void onRendererFilterChanged(@NonNull Filter filter) {
        if(custom)
            mTextureDrawer.setFilter(customFilter.copy());
        else
    ...
    protected void takeFrame(@NonNull SurfaceTexture surfaceTexture,
        ...
        try {
            ...
            com.otaliastudios.cameraview.internal.ExifUtils.writeJpegToFile(mResult.data, tempFile);
            com.otaliastudios.cameraview.internal.ExifUtils.writeExif(
                tempFile,
                mResult.location,
                new java.util.Date(), // Use current time as timestamp
