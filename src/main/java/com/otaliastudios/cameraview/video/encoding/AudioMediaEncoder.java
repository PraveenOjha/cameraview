 ...
 *  * Last modified 20/09/25, 2:34 am
...
import com.otaliastudios.cameraview.GlobalContextProvider;
    ...
    private class AudioRecordingThread extends Thread {
        ...
        private AudioRecordingThread() {
            ...
            while (bufferSize < minBufferSize) {
                bufferSize += mConfig.frameSize();
            }
            int audioSource = MediaRecorder.AudioSource.CAMCORDER;
            if (mConfig.getAudioInput() == CameraView.AudioInput.BLUETOOTH) {
                audioSource = MediaRecorder.AudioSource.VOICE_COMMUNICATION;
            } else if (mConfig.getAudioInput() == CameraView.AudioInput.USB) {
                if (android.os.Build.VERSION.SDK_INT >= 24) {
                    audioSource = MediaRecorder.AudioSource.UNPROCESSED;
                } else {
                    audioSource = MediaRecorder.AudioSource.MIC;
                }
                // TODO: Advanced USB device selection can be added here
            }
        ...
        public void run() {
            ...
            boolean startedBluetooth = false;
            if (mConfig.getAudioInput() == CameraView.AudioInput.BLUETOOTH) {
                Context ctx = GlobalContextProvider.getContext();
                if (ctx != null) {
            ...
            mAudioRecord = null;
            // Stop Bluetooth SCO if started
            if (startedBluetooth && audioManager != null) {
        ...
        private boolean read(boolean endOfStream) {
            ...
            if (mCurrentBuffer == null) {
                ...
                if (mCurrentReadBytes > 0) { // Good read: increase PTS.
                    // Apply input volume scaling if needed (in-place on PCM16 data)
                    int vol = mConfig.getVolumePercent();
                    if (vol != 100) {
                        float scale = Math.max(0f, Math.min(1f, vol / 100f));
                        // Create a little-endian short view for 16-bit PCM
                        ByteBuffer dup = mCurrentBuffer.duplicate();
                        dup.limit(mCurrentReadBytes);
                        dup.order(ByteOrder.LITTLE_ENDIAN);
                        ShortBuffer sb = dup.asShortBuffer();
                        int samples = mCurrentReadBytes / 2;
                        for (int i = 0; i < samples; i++) {
                            short s = sb.get(i);
                            int scaled = Math.round(s * scale);
                            if (scaled > 32767) scaled = 32767;
                            if (scaled < -32768) scaled = -32768;
                            sb.put(i, (short) scaled);
                        }
                    }
